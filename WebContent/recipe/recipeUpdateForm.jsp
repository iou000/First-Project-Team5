		...
		<div id="contents">
			<div class="innercon">
				<section class="conarea">
					...
					<form id="frmMassOrdInqrInf" method="POST" action="app?command=recipe_update" enctype="multipart/form-data" onsubmit="return checkUpdateForm()">
					    ...
					    <fieldset class="form-field">
					        ...
					        <dl>
					            ...
					            <dd>
					            	<label class="form-entry">
					            		<input type="text" name="title" title="레시피 제목 입력" maxlength="20" value="${recipeVO.title}" placeholder="레시피 제목을 작성해주세요!">
					            	</label>
					            </dd>
					        </dl>
					        <dl>
					            ...
					            <dd>
					            	<label class="form-entry">
					            		<input type="text" name="intro" title="레시피 소개 입력" maxlength="40" value="${recipeVO.intro}" placeholder="레시피 소개를 작성해주세요!" >					                 
					            	</label>
					            </dd>
					        </dl>
					        <dl>
								...
								<dd>
									<div class="col-input">
										<label>
											<input type="radio" name="category" value="한식" <c:if test='${recipeVO.category=="한식"}'> checked </c:if> />
											<span>한식</span>
										</label>
										<label>
											<input type="radio" name="category" value="양식" <c:if test='${recipeVO.category=="양식"}'> checked </c:if> />
											<span>양식</span>
										</label>
										<label>
											<input type="radio" name="category" value="일식" <c:if test='${recipeVO.category=="일식"}'> checked </c:if> />
											<span>일식</span>
										</label>
										<label>
											<input type="radio" name="category" value="중식" <c:if test='${recipeVO.category=="중식"}'> checked </c:if> />
											<span>중식</span>
										</label>
										<label>
											<input type="radio" name="category" value="분식" <c:if test='${recipeVO.category=="분식"}'> checked </c:if> />
											<span>분식</span>
										</label>
										<label>
											<input type="radio" name="category" value="간식" <c:if test='${recipeVO.category=="간식"}'> checked </c:if> />
											<span>간식</span>
										</label>
										<label>
											<input type="radio" name="category" value="디저트" <c:if test='${recipeVO.category=="디저트"}'> checked </c:if> />
											<span>디저트</span>
										</label>
									</div>
								</dd>
							...
							<dl>
					            ...
					            <dd>
					                <textarea placeholder="예: 물 1L, 마늘 한 쪽, 돼지고기 300g" name="ingredients" maxlength="500">${recipeVO.ingredients}</textarea>
					                <div class="infotxt">
					                    <ul>
					        ...
					        <dl>
					            ...
					            <dd>
					                <textarea placeholder="예: " name="details" maxlength="500">${recipeVO.details}</textarea>
					                <div class="infotxt">
					                    <ul>
...
<script type="text/javascript">
	...
	function checkUpdateForm() {
		if($('input[name=title]').val() == null || $('input[name=title]').val() == "") {
			...
			if(confirm('대표 이미지 없이 레시피를 생성할까요?')) return true;
